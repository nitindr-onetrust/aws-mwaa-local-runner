...
test-requirements)
   ...
   docker run -v $(pwd)/dags:/mnt/C/Airflow-Test/dags -v $(pwd)/plugins:/mnt/C/Airflow-Test/plugins -v $(pwd)/requirements:/mnt/C/Airflow-Test/requirements.txt -it amazon/mwaa-local:$AIRFLOW_VERSION test-requirements
   ;;
test-startup-script)
   BUILT_IMAGE=$(docker images -q amazon/mwaa-local:$AIRFLOW_VERSION)
   if [[ -n "$BUILT_IMAGE" ]]; then
...
package-requirements)
   ...
   docker run -v $(pwd)/mnt/C/Airflow-Test/dags -v $(pwd)/mnt/C/Airflow-Test/plugins -v $(pwd)/requirements:/mnt/C/Airflow-Test/requirements.txt -it amazon/mwaa-local:$AIRFLOW_VERSION package-requirements
   ;;   
build-image)
   build_image
   ;;
